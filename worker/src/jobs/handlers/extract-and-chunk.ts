...
import { chunkText, claude } from "@repo/lib";
...
export async function extractAndChunk(materialId: string): Promise<void> {
  ...
  const material = assertData(
    await supabase.from("source_materials").select().eq("id", materialId).single(),
  );
  ...
  if (material.mime_type === "application/pdf") {
    ...
    text = result.value;
  } else if (material.mime_type === "text/csv") {
  ...
  if (chunks.length > 0) {
    ...
    for (let i = 0; i < chunkRows.length; i += batchSize) {
      await supabase
        .from("source_chunks")
        .insert(chunkRows.slice(i, i + batchSize))
        .throwOnError();
    }
  }
  ...
  if (chunks.length > 0) {
    const insertedChunks = assertData(
      await supabase
        .from("source_chunks")
        .select()
      ...
      await Promise.all(
        batch.map(async (chunk) => {
          try {
            ...
            console.warn(
              `Failed to summarize chunk ${chunk.id} (index ${chunk.chunk_index}):`,
              err,
            );
          }
        }),
