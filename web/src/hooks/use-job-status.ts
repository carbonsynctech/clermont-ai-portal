    ...
    const poll = async () => {
      try {
        ...
        if (!res.ok) {
          ...
          console.warn(`[useJobStatus] Poll error ${consecutiveErrorsRef.current}/3, HTTP ${res.status}`);
        ...
        consecutiveErrorsRef.current += 1;
        console.error(`[useJobStatus] Poll network error ${consecutiveErrorsRef.current}/3:`, err);
        if (consecutiveErrorsRef.current >= 3) {
